...
public class Gamestate {
    ...
    public int diceRolled(DotsView dotsView){

        if(diceHandler.steps != 6 && !player[turn].isAnyUnlocked()) {
            nextTurn();
            Log.e("RES_VALUE", "diceHandler.steps != 6 && !player[turn].isAnyUnlocked(): " +"" );
            return 0;
        }
        else if(player[turn].hasSingleMovable(diceHandler.steps) && diceHandler.steps != 6) {
            Log.e("RES_VALUE", "player[turn].hasSingleMovable(diceHandler.steps) && diceHandler.steps != 6 " +"" );

            player[turn].move(player[turn].getSingleMovable(diceHandler.steps), diceHandler.steps,true,dotsView);return 0;
        }
        else if(!player[turn].checkMovementAvailable(diceHandler.steps) && !player[turn].allLastOrComplete(diceHandler.steps)) {
            Log.e("RES_VALUE", "!player[turn].checkMovementAvailable(diceHandler.steps) && !player[turn].allLastOrComplete(diceHandler.steps)" +"" );

            nextTurn();return 0;}
        else if(player[turn].allLastOrComplete(diceHandler.steps) && diceHandler.steps == 6) {
            Log.e("RES_VALUE", "player[turn].allLastOrComplete(diceHandler.steps) && diceHandler.steps == 6" +"" );

            diceHandler.rolled = false;
            return -1;
        }
        else if(player[turn].nothingMoveable(diceHandler.steps) && diceHandler.steps != 6) {
            Log.e("RES_VALUE", "player[turn].nothingMoveable(diceHandler.steps) && diceHandler.steps != 6" +"" );
    ...
    public void pieceClicked(int playernumber, int piece , DotsView dotview){
        player[playernumber].move(piece,diceHandler.steps, dotview);
    }
